...
// Vendored fork of grid 0.18.0; silence upstream's pre-existing lint noise so it
// does not pollute this workspace's build output as a path dependency.
    ...
    pub fn expand_rows(&mut self, rows: usize) {
        if rows > 0 && self.cols > 0 {
            let added = rows
                ...
                .expect("grid: rows * cols overflows usize");
            let new_len = self
                .data
                .len()
                .checked_add(added)
                .expect("grid: total size overflows usize");
    ...
    pub fn expand_cols(&mut self, cols: usize) {
        if cols > 0 && self.rows > 0 {
            let added = cols
                ...
                .expect("grid: cols * rows overflows usize");
            let new_len = self
                .data
                .len()
                .checked_add(added)
